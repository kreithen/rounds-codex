...
## LIVE ON THE APP STORE — 2026-08-26

**Rounds Codex: Clinical Atlas, iOS 1.0 (build 4), released.** Free, no IAP, 16+, Medical/Education,
Data Not Collected, no account, 826 MB, iPhone and iPad, fully offline.

Released quietly and deliberately: live on the store, announced to nobody. The launch work is in
`marketing-brief.md` (refreshed the same week) and `marketing/faculty-outreach-email.md`.

Eight days from a repo with no Xcode project in it to a shipped App Store app. Four builds, five
device-only bugs, three passes through review, two web deploys.
...
> ## 1.0.1 IS PREPARED AND WAITING ON THE MAC — 2026-09-16
>
> Three items, all ready. Nothing below needs another session; it needs Xcode.
>
> | item | state |
> |---|---|
> | **Universal Links entitlement** | ✅ `native/ios-project/App.entitlements` already claims `rounds-codex.netlify.app` alongside the brand domain. The file in the repo is correct; the **shipped 1.0(4) binary is not**, which is the whole bug. It needs to reach the Xcode project. |
> | **App icon** | ✅ `native/ios-project/AppIcon-1024.png` — 1024×1024, **opaque, zero alpha** (Apple refuses a transparent icon at upload rather than masking it). Generated by `scripts/build_app_icons.py`. |
> | **v138/v139 layout** | ✅ in the payload already — large-screen CSS and the condition-page rail both shipped to the web and are carried by the same tree. |
> | **v146/v147 two-pane** | ✅ also in the payload, and it is the reason to REBUILD before archiving: the 826.2 MB payload measured below was built before Level 2 landed. Above 1180px the nav is a left sidebar and the list stays beside what you opened, which is what makes an iPad screenshot set worth shooting. `preflight.sh ios` 13 passed / 0 failed / 0 skipped on the current tree. |
> | **the iPad listing** | ⚠ drafted, NOT submitted. `app-store-submission-draft.md` has the shot list and the copy. **Neither is true of the shipped 1.0(4)**, so both ride with this build — shooting them against the live app would be a 2.3.3 problem, not just a disappointment. |
>
> **Payload built and verified 2026-09-16: `preflight.sh ios` 10 passed, 0 failed, 0 skipped**, at
> 826.2 MB. That is the bytes that go in the archive, not the bytes they were derived from.
>
> **The Mac sequence** is `native/MAC-RUNBOOK.md` unchanged, with one addition: drop the new
> `AppIcon-1024.png` into the asset catalogue before archiving, and confirm the entitlement file the
> project references is the repo's, not a stale copy in the Xcode project's own folder — that
> distinction is exactly how 1.0 shipped claiming only roundscodex.com.
>
> ⚠ **Read `ANDROID-RUNBOOK.md` §3 before upgrading Capacitor for Android.** `@capacitor/android`
> and `@capacitor/core` majors must match, so a move to Capacitor 8 upgrades the **whole project,
> including iOS** — deployment target 14.0 → 15.0, dropping iOS 14 devices. It does not touch the
> live 1.0(4), but it binds this 1.0.1. Decide the order deliberately.

- [ ] **1.0.1 — THE APP ICON IS A CROPPED VERSION OF THE BRAND MARK.** Found 2026-09-16 by the
      physician while uploading the Play listing — "don't see the full circle". The emblem in
      `scripts/logo-trim.png` is a **closed ring** with the ECG trace running through it and out
      both sides; every shipped icon has the right arc missing, so it reads as a "C". Measured by
      the bounding box of everything brighter than the ground, since a closed ring is square: all
      four web icons came in at aspect **0.78**, and the identical number across four sizes is the
      tell that the crop happened **once upstream** and propagated. They were also off-centre by
      12–15%, which is why it was hard to see — it read as deliberate asymmetry.
      **Fixed on the web in v143** and on Play with `native/play-graphics/play-icon-512.png`, both
      generated by `scripts/build_app_icons.py` (aspect 1.02, centred within 0.6%).
      **iOS needs a build, not a listing edit** — the App Store icon comes from the binary. Fold it
      into this 1.0.1 alongside the entitlement fix and the v138/v139 layout work; the build is
      happening anyway, so it costs nothing extra. Until then the two stores show different marks.
      The 1024×1024 App Store icon must be **opaque with no alpha** (Apple rejects transparency),
      which the generator's full-bleed variant already produces.
