...
**Android App Links follow the same rule — settled 2026-09-16:** `assetlinks.json` goes on
**`rounds-codex.netlify.app`**, not the brand domain, for the same reason. `HANDOFF-android-app.md`
said roundscodex.com because it predates the domains decision; it is corrected.
...
- **App icons are GENERATED from `scripts/logo-trim.png`** by `scripts/build_app_icons.py`, never
  hand-made — five outputs (four web + the Play store icon), each with the corner treatment its
  platform wants. **Do not "fix" one icon by editing it**; fix the generator and re-run.
  **The lesson that bought this script:** every shipped icon carried a CROPPED mark with the right
  arc of the ring missing, so the emblem read as a "C" rather than the brand's closed circle. It
  survived from launch until the physician noticed it on 2026-09-16 while uploading to Play.
  **The guard is the bounding box's ASPECT** — a closed ring is square; all four shipped files
  measured **0.78**, and the identical number across four sizes is what proved the crop happened
  once upstream and propagated rather than four separate mistakes. A check that counted bright
  pixels, or tested the centroid, would have passed all four.
  Fixed in v143 (web) and `native/play-graphics/play-icon-512.png` (Play). **iOS still has it** —
  its store icon comes from the binary, so it is a 1.0.1 build item, not a listing edit.
  **The rest of the brand artwork was audited 2026-09-16 and is CLEAN** — `BRAND-ASSET-AUDIT.md`.
  The crop never spread past those files. It did surface two other inconsistencies: the USMLE
  module's logo was the **wordmark only, no emblem** (`usmle/assets/logo.png`, the same file as
  `preview/assets/logo.png`) — **fixed in v145** by `scripts/build_usmle_logo.py`, which generates
  it from the same `logo-trim.png` as the icons; and the **landing site's apple-touch-icon is a
  different icon design** from the app's — thick solid ring vs the glowing thin one. The second
  lives on `main` and belongs to that conversation.
  **Keying the lockup onto a page is not a resize.** `logo-trim.png` is RGB on a near-black ground
  and `.brandimg` carries a `drop-shadow`, which is cast by the **alpha shape** — so an opaque paste
  gives a glowing rectangle, and keying at alpha 0 is not enough either, because the corner patches
  read 10–24 and leave the rectangle at ~9%. Take the floor from the **corner patches, never the
  whole border**: the ECG trace leaves the frame at the bottom edge and reads 255 there.
- **Link-preview cards** — the `<head>`'s `og:`/`twitter:` block (v140, `scripts/add_og_tags.js`) is
  wrapped in **`<!--RC_OG-->` … `<!--/RC_OG-->`**, and a Netlify Edge Function
  (`netlify/edge-functions/route-previews.js`, generated by `scripts/build_route_previews.js`)
  swaps everything between them per route for a recognised preview crawler — 394 routes, v142.
  **Any new `og:` tag must go INSIDE the sentinels**: one placed after the closing sentinel survives
  the swap and, being later in the head, wins. `scripts/verify_route_previews.js` asserts exactly
  that, and is in `preflight.sh` under web mode.
  **Regenerate after any content change** (`node scripts/build_route_previews.js <web-clone> --apply`)
  — the table is baked from `content/*.json`, so a new condition ships without a card until you do.
  **It is gated on the crawler user-agent on purpose.** Rewriting HTML at the edge buffers and
  decodes the body; index.html is 756 kB raw against brotli on the wire, so transforming every
  request would cost a phone five times the bytes on the one route users actually open. A browser
  navigation returns early, untouched. An unrecognised agent falls back to the site-wide card, which
  is the pre-v142 behaviour — the failure direction is safe.
  **Which UA iOS Messages sends cannot be checked from a session** (no iOS, no WebKit). If a real
  share shows the generic card, the fix is one string in `CRAWLERS`.
