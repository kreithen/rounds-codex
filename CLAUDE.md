...
- **App icons are GENERATED from `scripts/logo-trim.png`** by `scripts/build_app_icons.py`, never
  ...
  **The rest of the brand artwork was audited 2026-09-16 and is CLEAN** — `BRAND-ASSET-AUDIT.md`.
  ...
  module's logo is the **wordmark only, no emblem** (`usmle/assets/logo.png`, the same file as
  `preview/assets/logo.png`), and the **landing site's apple-touch-icon is a different icon design**
  from the app's — thick solid ring vs the glowing thin one. The second lives on `main` and belongs
