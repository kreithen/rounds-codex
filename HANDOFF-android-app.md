...
> **DONE 2026-09-09, on `claude/native-android-app`.** Built and verified headless against
> `rounds-codex-app` a792eb4 / v132. What shipped, against the six items below:
>
> | item | outcome |
> |---|---|
> | 1 naming | `build_ios_payload.js` → **`build_native_payload.js --platform ios\|android`** (git mv, history kept). The old name survives as an alias that supplies `--platform ios`, because `MAC-RUNBOOK.md` step 2 and `preflight.sh` both name it and a runbook line that has been run four times should not break on a rename. `--platform` is **required, with no default** — the thing it selects is the check that stops an unshippable Android bundle. `build_ios_variant.js` and its `RC_IOS_VARIANT` marker are untouched, as instructed. |
> | 2 `fix_root_authority` | Kept, and **measured rather than assumed**. Running RC_ROOT's own expression over each origin in Chromium: `https://localhost` → href `https://localhost/` → RC_ROOT `https://localhost/` (correct); `capacitor://localhost` → RC_ROOT `capacitor://` (host eaten — the iOS bug). So it is provably inert on Android. Recorded in the script's own header. Scope of the claim: `new URL()` in desktop Chromium, not `location.href` in a real Android WebView. |
> | 3 `fix_usmle_link` | Kept for both. Unchanged. |
> | 4 service worker | **New `scripts/strip_service_worker.js`**: brace-walks the registration statement out of `index.html` (string-aware, asserting it neither under- nor over-runs into the click delegate that follows it) and deletes `sw.js`. `measure_bundle.js` resolves `sw.js` as reachable, so the payload's own sweep would have kept it — it has to go by name. Verifier checks added; see below. |
> | 5 safe areas | No payload change was needed or made: `env(safe-area-inset-*)` is the same mechanism in Android WebView, and the CSS block is already platform-neutral. `add_safe_area.js` now takes `--platform` **for its closing NOTE only** — it was printing iOS's `contentInset` instruction after an Android build, which is how someone sets a key that does not exist on their platform. The Android note carries the two-line `chrome://inspect` diagnostic and says plainly that the activity-side setting is **not verified from a container**. That remains the expected first real-device bug. |
> | 6 verifier | Three checks added to `verify_ios_variant.js`. **The count, not the controller, is the load-bearing one** — see the note beside them; I predicted the controller check would be decoration and measurement said otherwise, for a reason (`skipWaiting`/`clients.claim`) that a future edit to `sw.js` could remove. |
>
> **Both payloads are byte-identical** (`diff -rq` over the full trees), which is the claim §3 point 3
> rests on, now measured instead of asserted. Both pass **all 31 checks**. Against the unmodified web
> build the verifier fails **17 of 31**, all three service-worker checks among them — so the new
> guard is a guard and not decoration. (The header had claimed "11 of 19" for a long time and nobody
> had re-measured; corrected.)
>
> **A regression this exposed, fixed in the same commit.** Stripping `sw.js` broke two guards that
> assumed it exists: `preflight.sh` ran `verify_sw.js` against the payload, and `audit_app_e2e.js`
> hard-failed with *"sw.js could not be fetched"*. Both reported a **correct build as broken**, which
> is the failure mode that teaches people to ignore a suite. `verify_sw.js` now runs against the web
> tree it belongs to; `audit_app_e2e.js` reads the `RC_NO_SERVICE_WORKER` marker and asserts the
> opposite invariant on a native payload (detected on the marker, not on the absence of the file —
> absence is also what a truncated copy or a wrong `webDir` looks like, and it has to tell those
> apart). `preflight.sh` gained an **`android` mode**; `sh scripts/preflight.sh android <web-clone>`
> is 8 checks, all green.
>
> **The Play size rule is now enforced at build time.** `--platform android` without `--asset-packs`
> is a hard error naming the 200 MB base-module cap, refused in a second rather than after a Gradle
> build and an upload. With packs the payload is **84.2 MB, 115.8 MB of headroom**, and the stripped
> 741.9 MB is what §4.3 has to place. `--allow-oversize` exists for deliberate measurement and says so.
>
> **Not done here, and still open:** everything in §4.3 (the pack-vs-stream decision needs Android
> Studio), §4.4 and §4.6. `native/ANDROID-RUNBOOK.md` (§4.5) is not written yet.
