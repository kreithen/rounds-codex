...
'  /* full-screen overlay so the module covers the app (not a split-screen in-flow block) */',
'  #nclex-root{display:none;position:fixed;top:0;left:0;right:0;bottom:0;z-index:100000;overflow-y:auto;-webkit-overflow-scrolling:touch;background:#0f1419;}',
'  #nclex-close{display:none;position:fixed;top:16px;right:14px;z-index:100001;width:40px;height:40px;padding:0;border:none;border-radius:50%;background:rgba(20,26,34,.92);color:#e6edf3;font-size:22px;line-height:38px;text-align:center;cursor:pointer;box-shadow:0 4px 14px rgba(0,0,0,.5);}',
...
'    // Full-screen close control (a body sibling of #nclex-root, so the engine re-renders never remove it).',
'    var closeBtn = document.getElementById("nclex-close");',
'    if(!closeBtn){',
'      closeBtn = document.createElement("button"); closeBtn.id = "nclex-close";',
'      closeBtn.setAttribute("aria-label","Close NCLEX-RN practice"); closeBtn.innerHTML = "\\u00d7";',
'      closeBtn.onclick = function(){',
'        var r = document.getElementById("nclex-root"); if(r) r.style.display = "none";',
'        closeBtn.style.display = "none"; document.body.style.overflow = "";',
'      };',
...
'        if(r){ r.style.display = "block"; r.scrollTop = 0; }',
'        var c = document.getElementById("nclex-close"); if(c) c.style.display = "block";',
