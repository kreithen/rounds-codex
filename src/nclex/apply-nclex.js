...
'  /* Full-screen takeover: when open, the module covers everything (no split screen). */',
'  #nclex-root{display:none;}',
'  #nclex-root.nx-open{display:block;position:fixed;inset:0;z-index:9999;overflow:auto;',
'    background:#0f1419;-webkit-overflow-scrolling:touch;}',
'</style>',
'<script>',
dataSrc.trim(),
...
'(function(){',
'  "use strict";',
'  // Mount the module once, then add a Nursing-mode entry point that OPENS it full-screen.',
'  // IMPORTANT (fixes the split-screen bug): opening hides the host app view and shows the',
'  // module as a full-screen layer; the module\'s Back button restores the host view.',
'  //',
'  // Claude Code: wire openModule()/closeModule() into the app\'s OWN navigation if it has one',
'  // (e.g. root("nclex") registered in ROOTS, mirroring how the USMLE/Rx modules take over the',
'  // screen). The generic fallback below hides the app root container and shows #nclex-root.',
'  function appRoot(){',
'    // Best-effort handle on the host app\'s main view container to hide while the module is open.',
'    return document.getElementById("app") || document.querySelector("main") ||',
'           document.querySelector("#root, .app, .container");',
'  }',
'  function openModule(){',
'    var r = document.getElementById("nclex-root"); if(!r) return;',
'    var app = appRoot(); if(app){ app.setAttribute("data-nclex-prev-display", app.style.display||""); app.style.display = "none"; }',
'    r.classList.add("nx-open");',
'    window.scrollTo(0,0);',
'    // Register the exit hook so the module\'s Back button returns to the app.',
'    window.NCLEX.open(closeModule);',
'  }',
'  function closeModule(){',
'    var r = document.getElementById("nclex-root"); if(r) r.classList.remove("nx-open");',
'    var app = appRoot(); if(app){ app.style.display = app.getAttribute("data-nclex-prev-display")||""; }',
'  }',
'  function init(){',
'    if(!(window.NCLEX && window.NCLEX.buildNclexPatched && window.NCLEX_DATA)) { return setTimeout(init, 40); }',
'    window.NCLEX.buildNclexPatched(document);           // injects #nclex-root + scoped CSS',
'    var root = document.getElementById("nclex-root"); if(root) root.classList.remove("nx-open");',
'    if(!document.getElementById("nclex-entry")){',
'      var entry = document.createElement("div"); entry.id = "nclex-entry";',
'      var btn = document.createElement("button"); btn.textContent = "NCLEX-RN practice";',
'      btn.onclick = openModule;',
...
'    }',
'    // expose for the app to call from its own nav if preferred',
'    window.NCLEX.openModule = openModule; window.NCLEX.closeModule = closeModule;',
'  }',
