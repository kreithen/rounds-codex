...
### The biggest owned-channel gap, found 2026-08-05 — CLOSED 2026-09-15 (v140 + v142)

> **Both halves have shipped. The section below is kept as the record of what the problem was,
> because the second half is the part everyone gets wrong.**
>
> **v140** put the tags in `index.html`: description, `og:`, `twitter:`, and a 1200×630 card. That
> fixed "a bare grey URL" and left "all 183 conditions share one card" exactly as the warning below
> says it would.
> **v142** fixed that, with the Edge Function this section calls a real task — `netlify/edge-functions/
> route-previews.js`, generated by `scripts/build_route_previews.js` from `content/*.json`.
> **394 routes** carry their own title and description: 183 conditions (the tagline), 21 sections,
> 102 galleries (their page titles), 75 resident and guideline-year pages, 10 calculators, and the
> three index routes. `og:url` is now set per route, which v140 was right to omit while there was
> only one value for it.
> **It is gated on the crawler user-agent** and a browser navigation is passed through untouched.
> Rewriting HTML at the edge means buffering and decoding the body, and index.html is 756 kB raw
> against ~150 kB of brotli on the wire — so transforming every request would have paid for the card
> with five times the bytes on the one route users actually open.
> **Still on the site-wide image.** Every route gets its own *words*; they all share `og-card.jpg`.
> Per-gallery card art is a separate job and `scripts/build_og_card.py` records the reason it is not
> trivial: the brand faces are not installed here, so any type rendered into a card is the wrong
> wordmark. A card built from gallery page 1 with no type would avoid that — worth doing, not done.
...
2. ~~**Fix link previews.**~~ **Done** — v140 (the site-wide card) and v142 (per-route cards for
   394 routes). Every share button the product already had is now distribution. What is left of it
   is per-gallery card *art*; the words are all in place.
...
1. ~~**Fix link previews.**~~ **Shipped 2026-09-15**, both halves: v140 gave the site one card,
   v142 gave 394 routes their own. The one thing nobody in this session could test is which
   user-agent iOS Messages sends — there is no iOS and no WebKit in a container — so **the first
   real share to Messages is the physician's check.** If it shows the generic card, the fix is one
   string in `CRAWLERS` in `scripts/build_route_previews.js`.
