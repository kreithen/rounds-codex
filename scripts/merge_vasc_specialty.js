...
const TITLE = 'Top 60 Vascular Surgery Topics';
const BATCHES = 12;
const EXPECT = 60;
const STAGING = path.join(__dirname, '..', 'resident-staging', 'vasc');
...
if (R.data.some(d => d.sec === SEC)) errs.push(`resident.json already contains sec="${SEC}"`);
...
if (R.active.includes(SEC)) errs.push(`"${SEC}" is already active, which means content was merged before`);
